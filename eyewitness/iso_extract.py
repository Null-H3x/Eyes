...
def main() -> int:
    ...
    print("\n[1] Model contrast on the broad isomorph set (L=13, broad min_rep=3)")
    ...
    for name, fn in (("per-msg-progressive", cm.per_msg_prog_rows),
        ...
        r = ce.extract(M, base_len=13, broad_repeats=3, rows_fn=fn,
                       anchor_repeats=3)
        results[name] = r
    ...
    print("  READ: free-δ flags ~nothing (permissive — fits any isomorph). "
          "per-msg-")
    print("  progressive is SELECTIVE: it keeps a self-consistent subset and")
    print("  flags the rest. 'distinct << linked' => the alphabet is only linked,")
    print("  NOT ordered (free variables collapse) — ordering remains open.")
    ...
    if r.recovery_ratio >= 0.95:
        ...
        print("  Under-determined: linked symbols collapse onto "
              f"{r.positions_distinct} positions, so the order is NOT pinned.")
        print("  The isomorphs constrain the alphabet but do not order it — "
              "consistent")
        print("  with the prior finding that ordering is the open step.")
    ...
    print("  NOT achieved: the alphabet is only LINKED, not ORDERED — under either")
    print("    model the determined positions collapse (distinct << linked), so")
    print("    isomorphs alone do not pin the order (free variables remain).")
    print("  NOT decisive: free-δ also fits (permissive), so isomorphs alone cannot")
    print("    crown per-msg-progressive as THE model. The flagged set is either")
    print("    contamination or genuinely non-positional interrelation.")
