...
### `alphabet_sweep.py` — disposition structured alphabet families (22/22)
...
**Frame-composition layer.** The base sweep dispositions constructions over
the *trigram-value labeling* — the community's convention, not necessarily the
devs'. If they built a structured alphabet over a latent labeling `u = F(v)`,
the q we hunt is `g∘F`. The layer enumerates principled frames — identity,
line-reversal, **first-appearance order**, **frequency order** — composing
every family through each, in both orientations where the frame isn't
self-inverse (value→latent vs latent→value is unknowable convention). Proven
redundancies prune the run: `affine∘reverse ⊂ affine`, `power∘reverse ⊂
prepower`, so reverse only adds coverage for trigram/deals/keyword. The
efficacy selftest plants `q = g∘F` under a scrambled frame: the base sweep
misses it (z = −3.0, no survivors) while the composed sweep recovers the
mirror doublet at z = −43.5.

**Real-corpus result: the structured-alphabet corner is EXCLUDED, in every
enumerated frame.** Base frame: ~787k constructions, ~1.57M scored (q, drift)
hypotheses, zero survivors (deepest −4.78). Frame campaign: all six families ×
appearance/frequency in both orientations plus reverse where non-redundant —
another ~3.69M constructions, ~7.4M scored hypotheses, zero survivors
(deepest −4.90, a trigram×appearance candidate heading a *correlated* cluster;
bridge-read as due diligence → SUPPORT_ONLY with readability 0.0, the karelian
letter-soup mirage signature — dispositioned as fluctuation). Honest caveat,
now narrowed: exclusion covers the enumerated frames; a latent labeling
outside {identity, reverse, appearance, frequency} remains untested and the
layer accepts new frames as one-function plug-ins.
