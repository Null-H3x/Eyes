...
```bash
python3 selftest.py          # all six audits (~5 min; iso_relax dominates)
python3 selftest.py --fast   # skip the slow order_anneal plaintext read
```

Total audit coverage: **59 checks across 6 modules, all passing.**
...
### `plantlab.py` — shared ground-truth generators (9/9)
Single source of planted corpora so every audit attacks the *same* worlds. Five
model families (`pmp`, `pure`, `autokey1`, `chain_nz`, `alberti`), plaintext
coded into `Z_N` by a random injection, optional shared prefixes. KATs verify
each generator round-trips through its own inverse.
