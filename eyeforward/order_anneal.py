...
def reduce_to_residues(cts: Cts, q: Sequence[int], N: int,
                       drift: int = 1) -> List[List[int]]:
    """Residues r[t] = q[c[t]] - drift*t.

    drift=+1 is the per-message-progressive model.  drift=-1 covers a
    reverse-stepping variant; drift=0 a pure rotated mono-sub.  The chain
    test cannot pin drift down at real-corpus sparsity (model_power showed
    'pure' plants pass 6/6), so downstream readers should sweep it.
    Plaintext SIGN, by contrast, needs no parameter here: a Beaufort-style
    (-p) generator only relabels the canonical residues, and the learned
    substitution absorbs any relabeling (locked in by selftest check 6)."""
    return [[(q[c] - drift * t) % N for t, c in enumerate(ct)] for ct in cts]
...
def solve(cts: Cts, q: Sequence[int], N: int, *,
    ...
    res = reduce_to_residues(cts, q, N, drift=drift)
...
def selftest() -> List[Tuple[str, bool]]:
    ...
    # (6) SIGN ABSORPTION: a Beaufort plant (c = C[base - p + t]) reads
    #     WITHOUT any code change, because canonical residues are then a
    #     fixed relabeling of plaintext and the climb learns arbitrary
    #     relabelings.  This locks in the capability so a future "helpful"
    #     refactor to a frequency-pinned or fixed-direction map can't
    #     silently reintroduce the blind spot model_power warned about.
    plb = plantlab.gen("beaufort", seed=11, shared_prefix=20)
    repb = solve(plb.cts, plb.q, plb.N, bank=bank, restarts=12, iters=20000,
                 seed=2)
    jb = " ".join(repb["plaintext"]).lower()
    hb = sum(jb.count(w) for w in common)
    checks.append((f"beaufort plant reads via learned map "
                   f"(z={repb['z']}, hits={hb})",
                   repb["z"] > 6 and hb >= 20))
    return checks
