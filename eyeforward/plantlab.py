...
def gen(model: str, *, n_msgs: int = 9, msg_len: int = 115, N: int = N_DEFAULT,
        ...
        shared_prefix: int = 0) -> Plant:
    """Generate a plant. shared_prefix > 0 forces the first k characters of
    every message to be identical (models the shared openings)."""
    rng = random.Random(seed)
    pts = _chunks(text, n_msgs, msg_len, rng)
    if shared_prefix:
