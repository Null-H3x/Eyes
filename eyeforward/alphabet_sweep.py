...
#
# The base sweep dispositions constructions of q OVER THE TRIGRAM-VALUE
# LABELING of the glyphs -- the community's convention, not necessarily the
# devs'.  If they built a structured alphabet over a different latent
# labeling u = F(v), the q we hunt is q(v) = g(F(v)) = (g o F)(v).  This
# layer enumerates principled frames and composes every family through
# them.  Non-self-inverse frames are swept in BOTH orientations, because
# "value->latent" vs "latent->value" is a convention we cannot know.
#
# Redundancy notes (proved in the selftest where cheap):
#   affine o reverse  is affine (the constant absorbs);
#   power o reverse   lands in prepower (b=1);
#   prepower o reverse lands in prepower/power.
# So the reverse frame only adds coverage for trigram/deals/keyword.
# Unseen values (impossible on the real corpus -- Pyry C2, unbroken set --
# but possible on plants) are appended in value order, deterministically.
# --------------------------------------------------------------------------
def frame_identity(cts, N: int) -> np.ndarray:
    return np.arange(N)


def frame_reverse(cts, N: int) -> np.ndarray:
    return (N - 1) - np.arange(N)


def frame_appearance(cts, N: int) -> np.ndarray:
    """F[v] = rank of v's first appearance, corpus reading order."""
    seen: List[int] = []
    have = set()
    for ct in cts:
        for c in ct:
    ...
    F[np.array(seen)] = np.arange(N)
    return F


def frame_frequency(cts, N: int) -> np.ndarray:
    """F[v] = rank of v by descending corpus frequency, ties by value."""
    cnt = np.zeros(N, dtype=np.int64)
    for ct in cts:
        for c in ct:
            cnt[c] += 1
    order = np.lexsort((np.arange(N), -cnt))
    F = np.empty(N, dtype=np.int64)
    F[order] = np.arange(N)
    return F


FRAME_BUILDERS = {"identity": frame_identity, "reverse": frame_reverse,
                  "appearance": frame_appearance,
                  "frequency": frame_frequency}


def build_frames(cts, N: int, names: Sequence[str]
                 ) -> List[Tuple[str, np.ndarray]]:
    """Materialize (tag, F) frames, adding the inverse orientation for
    frames that are not self-inverse."""
    out: List[Tuple[str, np.ndarray]] = []
    for name in names:
        F = FRAME_BUILDERS[name](cts, N)
        out.append((name, F))
        Finv = np.empty(N, dtype=np.int64)
        Finv[F] = np.arange(N)
        if not (Finv == F).all():
            out.append((name + "_inv", Finv))
    return out


def compose(candidates: Iterable[Candidate], F: np.ndarray,
            tag: str) -> Iterator[Candidate]:
    """q = g o F: index the family candidate by the frame."""
    if tag == "identity":
        yield from candidates
        return
    for label, g in candidates:
        yield (f"{label} frame={tag}", np.asarray(g)[F])
...
def selftest() -> List[Tuple[str, bool]]:
    ...
    # (11) frame KATs on a hand corpus: values first seen 5,2,7,9 and
    #      frequency order 5(3x),2(2x),7,9 with unseen appended by value.
    tiny = [[5, 2, 5, 7], [2, 9, 5]]
    Fa = frame_appearance(tiny, 12)
    ok = (Fa[5], Fa[2], Fa[7], Fa[9]) == (0, 1, 2, 3) and \
         Fa[0] == 4 and Fa[11] == 11
    checks.append(("frame KAT: appearance (hand corpus)", ok))
    Ff = frame_frequency(tiny, 12)
    ok = (Ff[5], Ff[2]) == (0, 1) and Ff[7] == 2 and Ff[9] == 3
    checks.append(("frame KAT: frequency desc, ties by value", ok))
    ok = all(sorted(FB(tiny, 12).tolist()) == list(range(12))
             for FB in FRAME_BUILDERS.values())
    checks.append(("frames are valid permutations", ok))

    # (12) identity composition reproduces the base sweep bit-for-bit.
    ri = sweep(compose(aff[:20], np.arange(N), "identity"), pl.cts, N,
               null=null)
    rbase = sweep(aff[:20], pl.cts, N, null=null)
    checks.append(("identity frame == base sweep", ri["top"] == rbase["top"]))

    # (13) reverse-frame redundancy is REAL for affine: the composed q-set
    #      equals the base q-set (constant absorbs), justifying the pruned
    #      real run.  Support-relevant identity, checked as exact sets of
    #      support values on the plant corpus.
    R = frame_reverse(pl.cts, N)
    sup_rev = sorted(round(ordering_bridge.gsupport((q[R]).tolist(),
                                                    pl.cts, N, 1), 6)
                     for _, q in aff)
    sup_aff = sorted(round(ordering_bridge.gsupport(q.tolist(),
                                                    pl.cts, N, 1), 6)
                     for _, q in aff)
    checks.append(("affine o reverse == affine (support multiset)",
                   sup_rev == sup_aff))

    # (14) EFFICACY -- the check that earns the layer its keep: a plant
    #      built as g o F with a fixed scrambled frame is FOUND by the
    #      composed sweep and MISSED by the base sweep.
    rngF = np.random.default_rng(42)
    F0 = rngF.permutation(N)
    g17 = (17 * np.arange(N)) % N
    plF = _plant_with_q(g17[F0], seed=6)
    r_base = sweep(aff, plF.cts, N, seed=5)
    r_comp = sweep(compose(aff, F0, "F0"), plF.cts, N, seed=5)
    z_b = r_base["top"][0][0]
    z_c = r_comp["top"][0][0]
    top2f = {(lab.split(" frame=")[0], d) for _, d, lab in r_comp["top"][:2]}
    checks.append((f"frame efficacy: base misses (z={z_b:.1f}), composed "
                   f"finds the 17/66 doublet (z={z_c:.1f})",
                   len(r_base["survivors"]) == 0 and z_c < -10
                   and top2f == {("affine a=17", 1), ("affine a=66", -1)}))
...
def main() -> int:
    ...
    ap.add_argument("--frames", default="identity",
                    help=f"comma list from {sorted(FRAME_BUILDERS)}")
    ...
    frames = build_frames(cts, N, [f.strip()
                                    for f in args.frames.split(",")])
    for ftag, F in frames:
      for fam in args.families.split(","):
        gen = FAMILIES[fam.strip()]
        kw = {}
        if args.limit and fam.strip() in ("trigram", "keyword"):
            kw["limit"] = args.limit
        cand = compose(gen(N, **kw) if kw else gen(N), F, ftag)
        res = sweep(cand, cts, N, xp=xp, topk=args.topk, null=null)
        print(f"\n== {fam} frame={ftag}: scored {res['scored']}, "
