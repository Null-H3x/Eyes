...
WORKFLOWS = [
    ...
    ("EyeCrack — 26/52 deck sweep (range cuts + wiki header crib)", "Attack",
     "eyecrack", ["deck_sweep.py", "--presets"]),
    ...
    ("EyeCrack — alphabet brute-force with scoring (rotate/swap/random/exhaust)", "Attack",
     "eyecrack", ["alphabet_brute.py", "--mode", "random", "--samples", "5000",
                  "--top", "10"]),
    ("Rosetta — partial glyph→letter mapping propagator", "Attack",
     "eyecrack", ["rosetta.py", "--pin", "0:a"]),
    ("Refrain pipeline — compose → order_solve ranked report", "Attack",
     "eyecrack", ["refrain_pipeline.py", "--top", "15"]),
    ("Template phrase generator — bounded refrain enumeration", "Attack",
     "eyecrack", ["template_phrase.py", "--top", "20"]),
    ("Base search — clustered progressive-base hypothesis", "Attack",
     "eyecrack", ["base_search.py", "--mode", "auto", "--top", "10"]),
    ("Pos-0 base — position-0 indicator → base_m", "Attack",
     "eyecrack", ["pos0_base.py"]),
    ("Triplet base search — per-triplet progressive bases", "Attack",
     "eyecrack", ["triplet_base_search.py", "--mode", "auto", "--top", "5"]),
    ("Order bench — batch crib → ordering recovery", "Attack",
     "eyecrack", ["order_bench.py", "trueknowledgeofthegods", "--top", "10"]),
    ("Compose → order — anchored refrain shortlist", "Attack",
     "eyecrack", ["compose_order.py", "--top", "15"]),
    ("Ordering exhaust bench — Phase 2 residual search", "Attack",
     "eyecrack", ["exhaust_bench.py", "--phrase", "trueknowledgeofthegods"]),
    ("Tier-1 paranoia audit (cross-module edge cases)", "Validate",
     "noita_eye_core", ["tier1_audit.py"]),
    ("Tier-2 paranoia audit (Phase 2 crib/ordering tools)", "Validate",
     "noita_eye_core", ["tier2_audit.py"]),
...
def setup():
    ...
    if rc == 0 and have_venv():
        ...
        if not have_venv():
            ...
            print(DIM(f"  Tip: minimal Ubuntu/Debian images often need:"))
            print(DIM(f"    sudo apt install python{maj}.{min}-venv python3-pip"))
            print(DIM(f"  Then re-run Setup (s), or: python3 full-installer.py --no-venv"))
