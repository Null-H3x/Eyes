...
Usage:
    ...
    python3 refrain_sweep.py --phrase "someguesshere"
...
def _show_template(messages, N) -> int:
    ...
    print("=" * 70)
    print(f"Region {[(corpus_mod.load().labels[m], p) for m, p in region]}, L={L}")
...
<p class="meta">Generated {esc(meta.get('ts', ''))} — {meta.get('candidates', 0)} candidates, "
f"{meta.get('survivors', 0)} scored</p>
<table>
<thead><tr><th>phrase</th><th>off</th><th>stage</th><th>pinned</th>"
f"<th>cov</th><th>z</th><th>wcov</th><th>words</th></tr></thead>
...
def main() -> int:
    ...
    if args.wordlist:
        ...
        if not wl.is_absolute():
            wl = HERE / wl
        phrases.extend(rs.load_wordlist(str(wl)))

    if not phrases and not args.also_enum:
        print("Pass --phrase, --wordlist, --also-enum, or --show-template.")
        return 0

    print("=" * 70)
    print("EYECRACK — template-guided refrain sweep")
    print("=" * 70)
    print(f"Region L={rf.DEFAULT_LEN}; stages: pattern -> fits -> pin -> order_solve")

    model = ng.TrigramModel(args.alphabet, ng._ENGLISH)
    all_results = rs.sweep_candidates(
        M, phrases, N,
        alphabet=args.alphabet, model=model,
        restarts=args.restarts, iters=args.iters, n_null=args.null,
    ...
    pattern_ok = [r for r in all_results if r.stage_failed != "pattern"]
    fits_ok = [r for r in all_results if r.stage_failed not in ("pattern", "fits")]

    print(f"\nCandidates tried: {len(all_results)}")
    print(f"  passed pattern: {len(pattern_ok)}")
    print(f"  passed fits:    {len(fits_ok)}")
    print(f"  consistent pin: {len(scored)}")
    ...
    if args.html:
        ...
        _write_html(all_results[: args.top], Path(args.html_out), meta)
        print(f"\nWrote {args.html_out}")

    print("\nREAD: Most wordlist tokens fail at pattern/fits (expected). Survivors")
